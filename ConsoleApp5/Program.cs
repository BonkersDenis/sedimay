    ...
    internal class Program
    {
     ...
     // задание 2 
        ...
        static void Main()
        {
            ...
            var cook_book = FileData();
            foreach (var per in cook_book)
            {
                Console.WriteLine(per.Key);
                Console.WriteLine(per.Value.Count);

                foreach (var lol in per.Value)
                    Console.WriteLine($"{lol.name} | {lol.quantity} | {lol.measure}");

                Console.WriteLine();
            }
           
            var dishes = get_shop_list_by_dishes(new List<string> { "Омлет", "Фахитос", "Утка по-пекински" }, 2);
            foreach (var per in dishes)
            {
                Console.Write($"{per.Key} | ");

                foreach (var lol in per.Value)
                    Console.WriteLine($"{lol.quan} | {lol.meas}");
            }
